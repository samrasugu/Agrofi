...
class HomeService {
  Future<List<Booking>> fetchRequests({required BuildContext context}) async {
    ...
    // print("Booking requests:" + bookingsList.toString());
    return bookingsList;
  }
  ...
  Future<List<Booking>> fetchBookings({required BuildContext context}) async {
    ...
    // print("Active Bookings:" + bookingsList.toString());
    return bookingsList;
  }
  ...
  void changeStatus({
    ...
    required bool status
      ...
      final userProvider = Provider.of<UserProvider>(context, listen: false);
      try {
        http.Response res = await http.post(Uri.parse("$uri/api/tractorOwner/changestatus",),
        headers: <String, String> {
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': userProvider.user.token,
        },
